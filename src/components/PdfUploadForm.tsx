...
import { AlertCircle, Loader2, Check, X, Bug, FileText, Trash2 } from 'lucide-react';
...
export function PdfUploadForm() {
  ...
  const handleParse = async () => {
    if (!selectedProject || !source || !selectedFile) {
      setParseError('Please select a project, data source, and PDF file');
      return;
    }
    ...
    try {
      // Send the PDF file directly to the AI
      const listings = await parseCarListing(selectedFile);
      
      if (listings.length === 0) {
        throw new Error('No valid listings could be extracted from the PDF. Please check the format and try again.');
      }
    ...
    <div className="max-w-4xl mx-auto">
      <div className="bg-white shadow-sm rounded-lg p-6">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">New Entry - PDF Upload</h1>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="mt-1 flex items-center space-x-4">
              <label className="flex-1 relative cursor-pointer bg-white rounded-md font-medium text-primary hover:text-primary-dark focus-within:outline-none">
              ...
              <div className="flex">
                ...
                <div className="ml-3">
                  <p className="text-sm text-yellow-700">
                    Select your project and data source, then upload a PDF file containing car listings. The AI will automatically extract and parse the listings.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="flex justify-end space-x-4">
            {!parsedListings.length ? (
              <button
                type="button"
                onClick={handleParse}
                disabled={isSubmitting || !selectedFile || !source || !selectedProject}
